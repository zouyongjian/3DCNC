...
$action=$_GET["action"];
if($action=="getVerify"){
    $number=addslashes($_POST["telNum"]);
    $student=addslashes($_POST["student"]);
};
if(empty($number) || empty($student)) exit;
/*保存记录到数据库 start */
$mysql=new Mysql();
$mysql->setChar("utf8");
$mobile=addslashes($_POST["telNum"]);
$realname=addslashes($_POST["student"]);
$mobilepre=addslashes(substr($_POST["telNum"],0,7));
$sql="SELECT * FROM mobileloc WHERE mobilepre='$mobilepre'";
$rs = $mysql->getRow($sql);
$location=$rs['province'].$rs['city'];
$postTime=time();
$status="已发送";
$sql="INSERT INTO plugin_mobiletalk(realname,mobile,location,postTime,status) VALUES('$realname','$mobile','$location',$postTime,'$status')";
$mysql->query($sql);
/*保存记录到数据库 end */
$message = "姓名:".$student."——号码:".$number;
/*准备所有参数，进行排序，拼接*/
$name = "3D云课"; 
//$template_id = "91552223";      //必选参数，短信模板ID
$template_id="91552719";
$time=date("Y-m-d H:i");
$classname="CNC编程就业班";
$arr1=array("time"=>$time,"location"=>$location,"classname"=>$classname,"mobile"=>$mobile);
//$template_param = "{\"param1\":\"$name\",\"param2\":$number}"; //必选参数，模板匹配参数
//$template_param = "{'param1':".$name.",'param2':".$message."}";  //必选参数，模板匹配参数
//$template_param = "{\"param1\":\"$name\",\"param2\":\"$message\"}";
$template_param = json_encode($arr1);
